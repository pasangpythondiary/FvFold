...
import fvfold
from fvfold.preprocess import antibody_text_parser as ab_parser
...
def antibody_to_h5(pdb_dir,
    ...
    l3_set = h5_out.create_dataset('l3_range', (num_seqs, 2),
                                   ...
                                   dtype='uint16',
                                   fillvalue=-1)
    # Create a dataset with shape (15, 1024, 234)
    embs = h5_out.create_dataset('embs', (num_seqs, 1024, max_total_len), dtype='float')
    ...
    pairwise_geometry_set = h5_out.create_dataset(
        ...
        dtype='float',
        fillvalue=-1)
    # print("total max",max_total_len)

    dataloader_emb=embedding()
    
    for index,(file,i_m) in tqdm(enumerate(zip(pdb_files,dataloader_emb)),
        ...
        # sys.exit()
        # you can store the sequence_id in another dataset or use it as index
...
def cli():
    ...
    data_path = data_path+ 'final_antibody.h5'
    parser.add_argument(
        '--out_file',
        type=str,
        default=data_path,
